...
import { deletingSetting, gettingAllSettings } from "../../../redux/actions/settingAction";
...
const User = (props) => {
  ...
  const userDelete = async () => {
    ...
    if (res.status && res.status === 200) {
      ToggleNotification("DeleteSettingSuccess");
      setLoader(false);
      getData();
  ...
  const data = [
    ...
    { name: "Name", style: "center", width: "14.28%" },
    { name: "Address", style: "center", width: "14.28%" },
    ...
    <>
      ...
      <Row>
        <Col>
          <Card>
            <Card.Header>
              <Card.Title as="h5">Setting Data</Card.Title>
              <Button
                type="button"
                color="primary"
                className="float-right"
                onClick={(e) => history.push("/add-setting")}
              >
                Add
              </Button>
            </Card.Header>
                    ...
                    props.blogList.map((item, index) => {
                      return (
                        ...
                        >
                          ...
                          <td style={{textAlign: "center"}}>{item.address ? item.address.length > 30 ? item.address.slice(0,30) + '...' : item.address : "N/A"}</td>
                          ...
                          <td style={{textAlign: "center"}}>
                            <TableAction
                              data={tableData}
                              updateURL={`/update-setting/${item._id}`}
...
const mapStateToProps = (state) => {
  return {
    ...
    totalPage: state.setting.totalSetting,
    blogList: state.setting.settings,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    deletBlog: (id) => dispatch(deletingSetting(id)),
    changeStatus: (id, status) => dispatch(changingStatus(id, status)),
    getAllBlog: (page, limit) => dispatch(gettingAllSettings(page, limit))
  };
};
